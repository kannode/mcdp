...
from mcdp_web.resource_tree import ResourceThingViewEditor, ResourceThingViewEditorParse, ResourceThingViewEditorSave, \
    ResourceThingViewEditorGraph, ResourceThingsNew, V_EDIT_FANCY
...
#             'realpath': realpath, 
    ...
    def view_new_model_generic(self, e):
        ...
        url_edit = '../%s/views/%s/' % (new_thing_name, V_EDIT_FANCY)
...
def ajax_parse(e, editor):
    ...
    cache = editor.last_processed2

    make_relative = lambda s: editor.make_relative(e.request, s)
    library = library_from_env(e)
...
def process_parse_request(library, string, spec, key, cache, make_relative):
    ...
    for w in context0.warnings:
        if w.where is not None:
            tooltip = w.msg
            highlight = html_mark(highlight, w.where, "language_warning", tooltip=tooltip)
