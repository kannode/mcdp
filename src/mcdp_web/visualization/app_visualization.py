...
from mcdp_web.resource_tree import ResourceThingViewSyntax, ResourceThingViewNDPGraph, \
    ...
    ResourceThingViews, V_EDIT_FANCY
...
class AppVisualization(object):
    ...
    def config(self, config):
        ...
        config.add_view(self.view_dummy, context=ResourceThingViewDPGraph,
        ...
        config.add_view(self.view_dummy, context=ResourceThingViewNDPGraph,
                        renderer='visualization/model_ndp_graph.jinja2')
        config.add_view(self.view_model_ndp_repr, context=ResourceThingViewNDPRepr,
    ...
    def view_syntax(self, e):
        ...
        url_edit0 = ("/repos/%s/shelves/%s/libraries/%s/%s/%s/views/%s/" %
                     (e.repo_name, e.shelf_name, e.library_name, e.spec.url_part, e.thing_name, V_EDIT_FANCY))
