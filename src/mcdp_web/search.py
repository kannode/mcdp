...
class AppSearch(object):
    ...
    img#gravatar2 {
        ...
        margin-bottom: -4pt;
            ...
            desc = '%s User <a href="%s" class="highlight"><code>%s</code><a> (%s)' % (
                icon, url, username, user_struct.info.name)
            d = {'name': name,
                 'type': 'user',
                 'desc': desc,
                 'url': url}
        ...
        for e in stuff:
            name = '%s %s (Repo %s, shelf %s, library %s)' % (
                e.spec_name, e.thing_name, e.repo_name, e.shelf_name, e.library_name)
            url = '/repos/%s/shelves/%s/libraries/%s/%s/%s/views/syntax/' % (
                e.repo_name, e.shelf_name, e.library_name, e.spec_name, e.thing_name)
